...
class JavaBuilder(Builder):
    ...
    _language = "Java"
    _env = {'JAVAC': settings.JAVA_BINARY, 'JCFDUMP': settings.JCFDUMP}
        ...
        for dirpath, dirs, files in os.walk(env.tmpdir()):
            for filename in files:
                if filename.endswith(".class"):
                    [classinfo, _, _, _] = execute_arglist([settings.JAVAP, os.path.join(dirpath, filename)],
                                                           env.tmpdir(),
                                                           self.environment())
                    if string.find(classinfo,main_method) >= 0 or string.find(classinfo,main_method_varargs) >= 0:
    ...
    def libs(self):
        ...
        return ["-classpath", ".:" + (":".join([settings.JAVA_LIBS[lib]
                                                 for lib in required_libs if lib in settings.JAVA_LIBS]))]
    ...
    def build_log(self, output, args, filenames):
        ...
        return t.render(Context({'filenames': filenames, 'output': output,
                                 'cmdline': os.path.basename(args[0]) + ' ' +
                                            reduce(lambda parm, ps: parm + ' ' + ps, args[1:], '')}))
