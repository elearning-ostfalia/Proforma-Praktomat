...
class Builder(Checker):
    """ Build a program. This contains the general infrastructure to build a program with a compiler.  Specialized
    subclass are provided for different languages and compilers. """
    ...
    _output_flags = models.CharField(max_length=1000, blank=True, default="-o %s",
                                     help_text=_('Output flags. \'%s\' will be replaced by the program name.'))
    ...
    def run(self, env):
        ...
        #[output, error, process.returncode, timed_out]
        ...
        log = self.build_log(output, args, set(filenames).intersection([solutionfile.path()
