...
RXFAIL = re.compile(
    r"^(.*)(FAILURES!!!|your program crashed|cpu time limit exceeded|ABBRUCH DURCH ZEITUEBERSCHREITUNG|Could not find class|Killed|Fehler:|failures|Die Einreichung ist nicht richtig)(.*)$",
    re.MULTILINE)
...
class ScriptChecker(Checker):
    ...
    def run(self, env):
        ...
        result.set_passed((not error) and self.output_ok(output))
