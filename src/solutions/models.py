...
class Solution(models.Model):
    ...
    number = models.IntegerField(null=False, editable=False, help_text=_(
        "Id unique in task and user.Eg. Solution 1 of user X in task Y in contrast to global solution Z"))

    task = models.ForeignKey('tasks.task')
    author = models.ForeignKey(User, verbose_name="solution author")
    creation_date = models.DateTimeField(auto_now_add=True)

    accepted = models.BooleanField(default=False, help_text=_(
        'Indicates whether the solution has passed all public and required tests'))
    warnings = models.BooleanField(default=False, help_text=_(
        'Indicates whether the solution has at least failed one public and not required tests'))
    plagiarism = models.BooleanField(default=False,
                                     help_text=_('Indicates whether the solution is a rip-off of another one.'))
    final = models.BooleanField(default=False,
                                help_text=_('Indicates whether this solution is the last (accepted) of the author.'))
    ...
    def publicCheckerResults(self):
        ...
        return sorted(filter(lambda x: x.public(), self.checkerresult_set.all()),
                      key=lambda result: result.checker.order)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.number == None:
            self.number = (self.task.solution_set.filter(author=self.author).aggregate(Max('number'))[
                               'number__max'] or 0) + 1
        if self.final:
...
class SolutionFile(models.Model):
    ...
    def _get_upload_path(instance, filename):
        ...
        return 'SolutionArchive/Task_' + unicode(
            solution.task.id) + '/User_' + solution.author.username + '/Solution_' + unicode(
            solution.id) + '/' + filename

    solution = models.ForeignKey(Solution)
    file = models.FileField(upload_to=_get_upload_path, max_length=500, help_text=_(
        'Source code file as part of a solution or Zip file containing multiple solution files.'))
    ...
    def save(self, force_insert=False, force_update=False, using=None):
        ...
        if (self.file.name[-3:].upper() == 'ZIP') or self.file.name[-3:].upper() == 'JAR':
            ...
            for zip_file_name in zip.namelist():
                if not self.ignorred_file_names_re.search(zip_file_name):
                    ...
                    zip_file_name = zip_file_name if isinstance(zip_file_name, unicode) else unicode(zip_file_name,
                                                                                                     errors='replace')
                    new_solution_file.file.save(zip_file_name, File(temp_file),
                                                save=True)        # need to check for filenames begining with / or ..?
        else:
...
def get_solutions_zip(solutions):
    ...
    for solution in solutions:
        ...
        zip.writestr((base_name + '.project').encode('cp437'),
                     render_to_string('solutions/eclipse/project.xml', {'name': project_name}).encode("utf-8"))
...
def path_for_user(user):
    return user.get_full_name().encode('ascii', 'ignore').translate(None, non_ascii_letters) + '-' + str(
        user.mat_number) + '-' + str(user.id)
