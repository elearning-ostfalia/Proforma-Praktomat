...
class SolutionFileForm(ModelForm):
    ...
    def clean_file(self):
        ...
        if data:
            #contenttype = magic.from_buffer(data.read(1024), mime=True)
            contenttype = mimetypes.guess_type(data.name)[0] # don't rely on the browser:
            # data.content_type could be wrong or empty
